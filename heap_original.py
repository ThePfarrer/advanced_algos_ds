...
class DWayHeap(object):
    ...
    def first_leaf_index(self):
        print(f"Lenght of self is {len(self)} and dimensions are {self.D}")
        print(f"First leaf index {(len(self) - 2) // self.D + 1}")
    ...
    def _heapify(self, elements: List[Any], priorities: List[float]) -> None:
        ...
        print(f"First leaf index is {self.first_leaf_index()}")
        last_inner_node_index = self.first_leaf_index() - 1
        print(f"Last inner node index is {last_inner_node_index}")
